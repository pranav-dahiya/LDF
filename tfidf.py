...
def compute_idf(filenames):
    ...
    for word in vocabulary:
        idf[word] = len(filenames) / num_documents[word]
    return idf
...
if __name__ == "__main__":
    filenames = glob.glob("lingspam/lingspam_full/train/*.txt")
    global idf
    idf = compute_idf(filenames)
    filenames.extend(glob.glob("lingspam/lingspam_full/test/*.txt"))
    pool = Pool()
